...
The material balance associated with a continuous stirred tank reactor (CSTR) model for an example chemical is
...
\noindent where $v$ is the volumetric flow rate (m$^3$/s), $C_{A0}$ is the inlet concentration (mol/m$^3$), $C_A$ is the outlet concentration (mol/m$^3$), $r_A$ is the reaction rate (mol/m$^3$s), $V$ is the reactor volume (m$^3$), and $\tau$ is the residence time (s) of chemical $A$ in the reactor. The reaction rate $r_A$ is determined using the rate constant $k$ from Equation \ref{eq:rate-constant} multiplied by the concentration in the tank. A depiction of a CSTR system with its inlet and outlet flows is shown in Figure \ref{fig:reactor-cstr}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\textwidth]{figures/reactor-cstr.png}
    \caption{Representation of a continuous stirred-tank reactor (CSTR) system. Source: Wikipedia.}
    \label{fig:reactor-cstr}
\end{figure}

A series of continuous stirred-tank reactor (CSTR) models were used to represent the mixing (residence time) of the different feedstocks in the NREL fluidized bed reactor.
